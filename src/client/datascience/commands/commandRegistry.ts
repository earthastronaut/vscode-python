...
export class CommandRegistry implements IDisposable {
    ...
    public register() {
        ...
        this.registerCommand(Commands.GoToCellStart, this.goToCellStart);
        this.registerCommand(Commands.GoToCellEnd, this.goToCellEnd);
        this.registerCommand(Commands.MoveCellUp, this.moveCellUp);
    ...
    private async goToCellStart(): Promise<void> {
        const activeCodeWatcher = this.getCurrentCodeWatcher();
        if (activeCodeWatcher) {
            return activeCodeWatcher.goToCellStart();
        } else {
            return Promise.resolve();
        }
    }

    private async goToCellEnd(): Promise<void> {
        const activeCodeWatcher = this.getCurrentCodeWatcher();
        if (activeCodeWatcher) {
            return activeCodeWatcher.goToCellEnd();
        } else {
            return Promise.resolve();
        }
    }

    private async moveCellUp(): Promise<void> {
        const activeCodeWatcher = this.getCurrentCodeWatcher();
        if (activeCodeWatcher) {
            return activeCodeWatcher.moveCellUp();
        } else {
            return Promise.resolve();
        }
    }

    private async insertCellAboveCurrent(): Promise<void> {
        const activeCodeWatcher = this.getCurrentCodeWatcher();
        if (activeCodeWatcher) {
            return activeCodeWatcher.insertCellAboveCurrent();
        } else {
            return Promise.resolve();
        }
    }

    private async insertCellBelowCurrent(): Promise<void> {
        const activeCodeWatcher = this.getCurrentCodeWatcher();
        if (activeCodeWatcher) {
            return activeCodeWatcher.insertCellBelowCurrent();
        } else {
            return Promise.resolve();
        }
    }
